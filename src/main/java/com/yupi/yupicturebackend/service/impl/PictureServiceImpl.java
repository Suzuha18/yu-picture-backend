...
import com.yupi.yupicturebackend.api.aliyunai.model.CreatePictureOutPaintingTaskRequest;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        // spaceId 可能是查询某个空间的图片，也可能是查询公共图库的图片，或者查询所有图片，所以这里要分情况讨论
        ...
        // 时间
        // 注意，时间的查询一般是一个范围，所以这里用 >= 和 < 来查询 editTime 在 startEditTime 和 endEditTime 之间的图片
    ...
    /* 通过查出来的图片列表，对图片信息脱敏，在找到查询到的图片对应的用户进行脱敏，然后返回
     * @param page
     * @param request
     * @return
     */
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginuser) {
        ...
        if (oldPicture.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "图片已审核，无需重复审核");
        }
        // 4.更新图片状态，数据库操作
        Picture updatePicture = new Picture();
        BeanUtils.copyProperties(pictureReviewRequest, updatePicture);
        updatePicture.setReviewerId(loginuser.getId()); // 审核人id
        updatePicture.setReviewTime(new Date());
        boolean result = this.updateById(updatePicture);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "图片审核状态修改失败");
    }

    /**
     * 填充审核参数，管理员自动过审，非管理员创建或编辑都要改为待审核
     *
     * @param picture
     * @param loginUser
     */
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        for (Element imgElement : imgElementList) {
            ...
            if (StrUtil.isNotBlank(namePrefix)) {//设置图片名称，序号连续递增
